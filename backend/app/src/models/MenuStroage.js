...
class MenuStorage {
  ...
  static getMenuDetail(menu_id) {
    return new Promise((resolve, reject) => {
      const query =
        "select menu.menu_name,menu.roman_name,menu.img_url, menu.menu_price, menu.menu_ing FROM menu WHERE menu.id = ?;";
  ...
  static async save(menu_info, rest_id) {
    ...
    return new Promise((resolve, reject) => {
      const menuquery = `INSERT INTO menu(rest_id,menu_name,kor_name, roman_name, menu_price,img_url,menu_ing)
